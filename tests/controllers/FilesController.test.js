...
describe('+ FilesController', () => {
  ...
  const mockUser = {
    email: 'yianjr@gmail.com',
    password: 'akobo_akobo',
  };
  /**
   * 3 sample files
   * + 1 -> file
  ...
  before(function (done) {
    this.timeout(10000);
    emptyDatabaseCollections(() =>
      signUp(mockUser, () => signIn(mockUser, done))
    );
    emptyFolder(baseDir);
  });
  ...
  describe('+ GET: /files', () => {
    ...
    it('+ Fetches first page with no page query', function (done) {
      ...
      request
        ...
        .end((requestErr, res) => {
          ...
          expect(res.body.some((file) => file.name === mockFiles[0].name)).to.be
            .true;
          expect(res.body.some((file) => file.name === mockFiles[1].name)).to.be
            .true;
          done();
        });
    });
    ...
    it('+ Fetches first page with no page query and parentId', function (done) {
      this.timeout(5000);
      request
        .get(`/files?parentId=${mockFiles[1].id}`)
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
          ...
          expect(res.body.some((file) => file.name === mockFiles[2].name)).to.be
            .true;
          done();
        });
    });
    ...
    it('+ Returns empty list for a parentId of a file', function (done) {
      this.timeout(5000);
      request
        .get(`/files?parentId=${mockFiles[0].id}`)
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
    ...
    it('+ Returns empty list for unknown parentId', function (done) {
      this.timeout(5000);
      request
        .get('/files?parentId=34556ea6727277193884848e')
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
  ...
  describe('+ PUT: /files/:id/publish', () => {
    ...
    it('+ Succeeds if file is linked to user', function (done) {
      this.timeout(5000);
      request
        .put(`/files/${mockFiles[0].id}/publish`)
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
  ...
  describe('+ PUT: /files/:id/unpublish', () => {
    ...
    it('+ Succeeds if file is linked to user', function (done) {
      this.timeout(5000);
      request
        .put(`/files/${mockFiles[0].id}/unpublish`)
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
